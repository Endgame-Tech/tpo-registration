...
export async function generateMemberId(profileData, count = 5) {
  const states = {
  ...
  if (exists && count > 0) {
    return generateMemberId(profileData, count - 1); // retry
  }
...
async function checkMemberId(member_id) {
  const existingUser = await User.findOne({ member_id });
  return !!existingUser; // true if found
}
